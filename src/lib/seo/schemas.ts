...
export function organizationSchema() {
  return {
    ...
    logo: `${SITE.url}/logo.svg`, description: SITE.description,
    areaServed: { '@type': 'Country', name: 'España' },
    contactPoint: { '@type': 'ContactPoint', telephone: SITE.phone, email: SITE.email, contactType: 'customer service', availableLanguage: 'Spanish' },
  };
}
export function serviceSchema(name: string, description: string, areaServed?: string) {
  return {
    ...
    areaServed: areaServed ? { '@type': 'City', name: areaServed } : { '@type': 'Country', name: 'España' },
    serviceType: name,
  };
}
export function localBusinessSchema(serviceName: string, city: string, lat: number, lng: number) {
  return {
    ...
    name: `${SITE.name} — ${serviceName} en ${city}`, description: `${serviceName} para comunidades de vecinos en ${city}`,
    url: SITE.url, telephone: SITE.phone, email: SITE.email,
