...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE_URL}/empleo/`, lastModified: now, changeFrequency: 'weekly', priority: 0.7 },
  ...
  // Pillar pages (6)
  const pillarPages: MetadataRoute.Sitemap = SERVICIOS.map((s) => ({
  ...
  // City landing pages (6 × 14 = 84)
  const cityPages: MetadataRoute.Sitemap = SERVICIOS.flatMap((s) =>
    CIUDADES.map((c) => ({
      url: `${BASE_URL}/servicios/${s.slug}/${c.slug}/`,
      lastModified: now,
