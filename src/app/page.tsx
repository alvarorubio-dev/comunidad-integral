          ...
          <div className="grid gap-16 lg:grid-cols-5 lg:items-end">
            ...
            <div className="lg:col-span-2">
              <div className="rotate-1 bg-[var(--color-crema-50)] p-6 shadow-2xl">
                <p className="font-mono text-base uppercase tracking-[0.1em] text-[var(--color-ink-soft)]">
                  Tablón de servicios
                </p>
                <div className="laton-rule mt-3" />
                <ul className="mt-4 space-y-3">
                  {SERVICIOS.slice(0, 4).map((s) => (
                    <li key={s.n} className="flex items-baseline gap-3 text-base">
                      <span className="font-mono text-base text-[var(--color-oxido-500)]">{s.n}</span>
                      <span className="text-[var(--color-ink)]">{s.nombre}</span>
                    </li>
                  ))}
                </ul>
                <p className="mt-4 font-mono text-base text-[var(--color-ink-soft)]">
      ...
      <section className="mx-auto max-w-4xl px-6 py-24 sm:px-8">
        ...
        <div className="mt-12 border-t border-[var(--color-crema-200)]">
          {SERVICIOS.map((s) => (
            ...
            >
              <span className="font-mono text-base text-[var(--color-laton-600)]">{s.n}</span>
              <div className="flex-1">
                <h3 className="font-[var(--font-display)] text-xl font-medium text-[var(--color-ink)] group-hover:text-[var(--color-oxido-600)]">
                  {s.nombre}
                </h3>
                <p className="mt-1 text-base text-[var(--color-ink-soft)]">{s.desc}</p>
              </div>
          ...
          <div className="mt-12 space-y-10">
            {ACTAS.map((a, i) => (
              <div key={i} className="flex gap-5">
                <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full border-2 border-[var(--color-oxido-500)] font-[var(--font-display)] text-base font-semibold text-[var(--color-oxido-600)]">
                  {a.firma.split('.')[0]}
                </div>
                <div>
                  ...
                  <p className="mt-2 font-mono text-base text-[var(--color-ink-soft)]">
                    {a.firma} — {a.cargo}
                  </p>
                </div>
              </div>
